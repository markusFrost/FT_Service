	...
	public BookAdapter (Activity context)
		{
			...
			list = AppContext.dbAdapter.getBooks(ShPrefUtils.getUserId(context));
	...
	*/
    ...
    public void Search(String name)
    {
        ...
        if ( books.size() == 0)
        {
            for ( Book b : list)
            {
                if (b.Author.toLowerCase().contains(name))
                {
                    books.add(b);
                }
            }
	...
	public List<Book> Read()
	{
		...
		{
			// TODO Auto-generated method stub
			return AppContext.dbAdapter.getBookById(ShPrefUtils.getUserId(context),list.get(position).Id);
		}
